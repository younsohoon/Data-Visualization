...
cat_length_avg <- cat_length %>%
  group_by(Category) %>%
  summarize(AvgLength = mean(Length2, na.rm = TRUE))

cat_length_g <- ggplot(cat_length_avg, 
                       aes(x = AvgLength, 
                           y = reorder(Category, AvgLength))) +
  geom_point(color = "red", 
             size = 2) +
  geom_segment(aes(x = 0, 
                   xend = AvgLength, 
                   ...
                   yend = Category),
               color = "azure4") +
  labs(x = "Average Episode Length (min)",
       y = "Category",
       title = "Average Episode Length by Category") +
  geom_text(aes(label = round(AvgLength, 1)), 
            position = position_stack(vjust = 0.5), 
            size = 2.5, 
            color = "red") +
  theme_bw()
